...
def Multi_plication(image, weight, bias, dim_0, dim_1, device0):
    @tf.custom_gradient
    def _lambda():
        return Multi_plication_real(image, weight, bias, dim_0, dim_1, device0)
    return _lambda()

def Multi_plication_real(image, weight, bias, dim_0, dim_1, device0):
    out = gp_apis.gp_Multi_plication(image, weight, bias, dim_0, dim_1, device0)
    def grad():
        return gp_apis.gp_Multi_plication(image, weight, bias, dim_0, dim_1, device0)
    return out, grad

def Trans_pose(input, dim_0, dim_1, device0):
    @tf.custom_gradient
    def _lambda(X):
        return Trans_pose_real(X, dim_0, dim_1, device0)
    return _lambda(input)

def Trans_pose_real(input, dim_0, dim_1, device0):
    out = gp_apis.gp_Trans_pose(input, dim_0, dim_1, device0)
    def grad(dZ):
        return gp_apis.gp_Trans_pose(dZ, dim_0, dim_1, device0)
    return out, grad

def Matrix_multiplication(image, weight, dim_0, dim_1, device0):
    @tf.custom_gradient
    def _lambda():
        return Matrix_multiplication_real(image, weight, dim_0, dim_1, device0)
    return _lambda()

def Matrix_multiplication_real(image, weight, dim_0, dim_1, device0):
    out = gp_apis.gp_Matrix_multiplication(image, weight, dim_0, dim_1, device0)
    def grad():
        return gp_apis.gp_Matrix_multiplication(image, weight, dim_0, dim_1, device0)
    return out, grad
